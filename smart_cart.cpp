...
void *DB_start(void *data){
	...
	//MODE = DELETE;
	char barcode[14];
	char note_barcode1[14] = "5060214370172";
	char raspberry_box[14] = "8809577081588";
	char drink[14] = "8801056042134";
	...
	if(MODE == INSERT){
		...
		while(1){ 
			...
			if(strcmp(barcode, note_barcode1)==0){
	
				sprintf(query, "insert into product values('%s',%d,'%s')",barcode,3000,product_name);
				printf("입력 된커리 문: %s\n",query);
			...
			else if(strcmp(barcode, raspberry_box)==0){
			...
			else if(strcmp(barcode, drink)==0){
	...
	if(MODE == DELETE){
		...
		while(1){ 
			...
			if(strcmp(barcode, note_barcode1)==0){
				...
				sprintf(query, "delete from product where barcode like '%s'",barcode);
				printf("입력 된커리 문: %s\n",query);
